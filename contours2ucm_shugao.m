function [ucm] = contours2ucm_shugao_org(pb, or, fmt)
...
[ws_wt] = create_finest_partition(pb, or);
...
function [ws_wt] = create_finest_partition(pb, or)
...
for e = 1 : numel(contours.edge_x_coords)
    ...
    if v1(2) == v2(2),
        ...
        angles(e) = atan((v1(1)-v2(1)) / (v1(2)-v2(2))) * 180 / pi;
    end
end

angles(angles < 0) = angles(angles < 0) + 90;
...
for e = 1 : numel(contours.edge_x_coords)
    ...
    for p = 1 : numel(contours.edge_x_coords{e}),
        ws_wt(contours.edge_x_coords{e}(p), contours.edge_y_coords{e}(p)) = ...
            max(pb(contours.edge_x_coords{e}(p), contours.edge_y_coords{e}(p)) * (cos((or(contours.edge_x_coords{e}(p), contours.edge_y_coords{e}(p)) - angles(e))/180)), ...
            ws_wt(contours.edge_x_coords{e}(p), contours.edge_y_coords{e}(p)));
    end
    v1=contours.vertices(contours.edges(e,1),:);
    v2=contours.vertices(contours.edges(e,2),:);
    %fprintf('%f %f\n', or(v1(1),v1(2)) - angles(e), or(v2(1),v2(2)) - angles(e));
    ws_wt(v1(1),v1(2))=max( pb(v1(1),v1(2)) * (cos((or(v1(1),v1(2)) - angles(e))/180)),ws_wt(v1(1),v1(2)));
    ws_wt(v2(1),v2(2))=max( pb(v2(1),v2(2)) * (cos((or(v2(1),v2(2)) - angles(e))/180)),ws_wt(v2(1),v2(2)));
end
ws_wt=double(ws_wt);
%%
